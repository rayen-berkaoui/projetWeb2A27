...
function editCampaign(id) {
    fetch(`/2A27/view/admin/pages/get_campaign.php?id=${id}`)
        ...
        .catch(error => {
            ...
            showToast('Error loading campaign: ' + (error.error || error.message), 'error');
        });
}
...
function validateForm(event) {
    ...
    .then(data => {
        if (data.success) {
            showToast(data.message || 'Campaign added successfully', 'success');
    ...
    .catch(error => {
        console.error('Error:', error);
        showToast('Error adding campaign: ' + error.message, 'error');
    })
    .finally(() => {
...
function editPartner(partnerId) {
    fetch(`/2A27/view/admin/pages/get_partner.php?id=${partnerId}`)
        ...
        .catch(error => {
            console.error('Error:', error);
            showToast('Error loading partner details: ' + error.message, 'error');
        });
}
...
function validatePartnerForm(event) {
    ...
    .then(data => {
        ...
        closePartnerModal();
        closeCampaignDetails();
        setTimeout(() => location.reload(), 1000); // Refresh the page to show updated data
    })
    .catch(error => {
        console.error('Error:', error);
...
function exportCampaignsAsPDF() {
    ...
    form.method = 'POST';
    form.action = '/2A27/view/admin/pages/export_campaign_pdf.php';
