...
<body>
    <div class="content-area">
        <h2>Edit Commentaire</h2>
        <form method="POST" action="/2A27/admin/commentaires/edit/<?= htmlspecialchars($commentaire['id_commentaire']) ?>">
            <div class="form-group">
                <label for="contenu">Content</label>
                <textarea name="contenu" id="contenu" class="form-input" required><?= htmlspecialchars($commentaire['contenu']) ?></textarea>
            </div>

            <div class="form-group">
                <label for="auteur">Author</label>
                <input type="text" name="auteur" id="auteur" class="form-input" value="<?= htmlspecialchars($commentaire['auteur']) ?>" required>
            </div>

            <div class="form-group">
                <label for="date">Date</label>
                <input type="date" name="date" id="date" class="form-input" value="<?= htmlspecialchars($commentaire['date']) ?>" required>
            </div>
            ...
            <div class="form-group">
                ...
                <select name="avis_id" id="avis_id" class="form-input" required>
                    <?php foreach ($avisList as $avis): ?>
                        <option value="<?= htmlspecialchars($avis['id_avis']) ?>" <?= $commentaire['avis_id'] == $avis['id_avis'] ? 'selected' : '' ?>><?= htmlspecialchars($avis['titre']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
